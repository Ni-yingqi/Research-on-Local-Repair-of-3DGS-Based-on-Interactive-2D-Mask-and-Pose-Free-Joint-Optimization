...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, train_test_exp, separate_sh):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        raw_depth = render_pkg["depth"].detach().float()
        ...
        np.save(os.path.join(depths_path, '{0:05d}'.format(idx) + ".npy"), raw_depth.cpu().numpy())
        if idx == 0:
            torchvision.utils.save_image(depth, os.path.join(depths_path, "depth.png"))
            np.save(os.path.join(depths_path, "depth.npy"), raw_depth.cpu().numpy())
